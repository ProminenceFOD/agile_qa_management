...
for (const line of lines) {
  if (line.startsWith('# ')) {
    // Heading 1
    rtf += '\\fs36\\b ' + line.substring(2).replace(/[\\{}]/g, '\\$&') + '\\b0\\fs22\\par\\par\n';
  } else if (line.startsWith('## ')) {
    // Heading 2
    rtf += '\\fs32\\b ' + line.substring(3).replace(/[\\{}]/g, '\\$&') + '\\b0\\fs22\\par\\par\n';
  } else if (line.startsWith('### ')) {
    // Heading 3
    rtf += '\\fs28\\b ' + line.substring(4).replace(/[\\{}]/g, '\\$&') + '\\b0\\fs22\\par\n';
  } else if (line.startsWith('#### ')) {
    // Heading 4
    rtf += '\\fs24\\b ' + line.substring(5).replace(/[\\{}]/g, '\\$&') + '\\b0\\fs22\\par\n';
  } else if (line.startsWith('- ') || line.startsWith('* ')) {
    // Bullet point
    rtf += '\\tab\\bullet ' + line.substring(2).replace(/[\\{}]/g, '\\$&') + '\\par\n';
  } else if (line.match(/^\d+\. /)) {
    // Numbered list
    const text = line.replace(/^\d+\. /, '');
    rtf += '\\tab ' + line.match(/^\d+/)?.[0] + '. ' + text.replace(/[\\{}]/g, '\\$&') + '\\par\n';
  } else if (line === '---') {
    // Horizontal rule
...
console.log('✅ This file will open in Microsoft Word, Google Docs, LibreOffice, etc.');
