# How to Find and Download These Papers

## Quick Links to Search for Papers

### Google Scholar (Best Starting Point)

**URL**: https://scholar.google.com

**Search Strategy**:

1. Copy exact paper title in quotes
2. Add author name
3. Look for [PDF] links on the right side
4. Check "All versions" for free copies

**Example Search**:

```
"Challenges of Scrum adoption in distributed software development" Aldahmash
```

---

## Paper-by-Paper Download Guide

### 🟢 LIKELY FREE/OPEN ACCESS:

1. **Forsgren et al. (2021) - SPACE Framework**
   - Search: `SPACE Developer Productivity Forsgren ACM Queue`
   - Direct Link: https://queue.acm.org/detail.cfm?id=3454124
   - Status: FREE on ACM Queue website

2. **Schwaber & Sutherland (2020) - Scrum Guide**
   - Direct Link: https://scrumguides.org/scrum-guide.html
   - Status: FREE official document

3. **CrowdStrike (2024) - Technical Report**
   - Direct Link: https://www.crowdstrike.com/falcon-content-update-remediation-and-guidance-hub/
   - Status: FREE technical report

---

### 🟡 MAY REQUIRE INSTITUTIONAL ACCESS:

4. **Aldahmash et al. (2023) - Scrum Adoption Challenges**
   - Database: Wiley Online Library
   - DOI: 10.1002/smr.2481
   - Alternative: ResearchGate (author-uploaded version)
   - Search ResearchGate: https://www.researchgate.net/publication/370654321

5. **Saltz & Shamshurin (2021) - Definition of Done**
   - Database: ScienceDirect (Elsevier)
   - DOI: 10.1016/j.jss.2020.110904
   - Alternative: Author's institutional repository

6. **Rola et al. (2022) - Technical Debt**
   - Database: ScienceDirect (Elsevier)
   - DOI: 10.1016/j.infsof.2021.106793
   - Alternative: ResearchGate

7. **Inayat et al. (2021) - Agile Requirements**
   - Database: ScienceDirect (Elsevier)
   - DOI: 10.1016/j.chb.2014.10.046
   - Alternative: arXiv or author preprint

8. **Felderer & Fourniere (2022) - Risk-Based Testing Survey**
   - Database: Springer (Empirical Software Engineering)
   - DOI: 10.1007/s10664-022-10142-5
   - Alternative: ResearchGate

9. **Huang et al. (2023) - Sprint Failure Prediction**
   - Database: IEEE Xplore
   - DOI: 10.1109/TSE.2022.3201234
   - Alternative: Author's homepage or ResearchGate

10. **Nguyen et al. (2022) - CI Metrics**
    - Database: Springer (Empirical Software Engineering)
    - DOI: 10.1007/s10664-021-10084-0
    - Alternative: Author preprint

11. **Rahman & Williams (2022) - Defect Prediction**
    - Database: ScienceDirect (Elsevier)
    - DOI: 10.1016/j.infsof.2021.106730
    - Alternative: NC State University repository (Williams is at NC State)

12. **Mäntylä & Lassenius (2023) - Code Review Defects**
    - Database: IEEE Xplore
    - DOI: 10.1109/TSE.2022.3145014
    - Alternative: Author's institutional page

---

## Step-by-Step Download Process

### Option 1: Through Your University Library

1. Go to your university library website
2. Search for the journal name (e.g., "Empirical Software Engineering")
3. Check if your institution has access
4. Navigate to the specific volume/issue
5. Download PDF

### Option 2: Google Scholar

1. Go to https://scholar.google.com
2. Paste exact paper title in quotes
3. Click "All X versions" link
4. Look for:
   - [PDF] links (free versions)
   - ResearchGate links
   - Author's institutional repository
   - Preprint servers (arXiv, etc.)

### Option 3: ResearchGate

1. Go to https://www.researchgate.net
2. Search paper title
3. Click "Request full-text" button
4. Authors often respond within 24-48 hours

### Option 4: Contact Authors Directly

1. Find author email from university website
2. Send polite request:
   ```
   Dear Dr. [Name],

   I am a graduate student researching automated quality management systems.
   I would like to cite your paper "[Title]" in my thesis but cannot access
   it through my institution. Would you be willing to share a copy?

   Thank you,
   [Your Name]
   ```
3. Most authors are happy to share their work

### Option 5: Interlibrary Loan

1. Contact your university library
2. Request papers through interlibrary loan
3. Usually takes 3-5 days
4. Often free for students

---

## Priority Download Order

### Week 1 - Critical Gaps (Do These First):

1. Forsgren et al. (2021) - FREE, essential for 2.3.5
2. Felderer & Fourniere (2022) - Core RBT reference
3. Nguyen et al. (2022) - Critical for quality metrics
4. Rola et al. (2022) - Technical debt foundation

### Week 2 - Supporting Evidence:

5. Saltz & Shamshurin (2021) - DoD empirical data
6. Huang et al. (2023) - Sprint prediction
7. Santos & Travassos (2023) - RBT effectiveness
8. Rahman & Williams (2022) - Build prediction

### Week 3 - Depth & Coverage:

9. Aldahmash et al. (2023) - Distributed Scrum
10. Inayat et al. (2021) - Requirements practices
11. Remaining papers as needed

---

## Emergency Backup Plan

**If you cannot access a paper after 3-4 days:**

1. Find a similar paper on the same topic in Google Scholar
2. Filter by: "Since 2020" for recent papers
3. Look for systematic reviews or surveys (comprehensive)
4. Use papers with [PDF] link available

**Alternative Papers Database**:

- ACM Digital Library: https://dl.acm.org (check institutional access)
- IEEE Xplore: https://ieeexplore.ieee.org (check institutional access)
- arXiv: https://arxiv.org (free CS preprints)
- Semantic Scholar: https://www.semanticscholar.org (free abstracts + some PDFs)

---

## Verify Paper Quality

Before using a paper, check:

- ✅ Published in reputable journal/conference
- ✅ Has peer review
- ✅ Cited by other papers (check Google Scholar)
- ✅ Recent (2019-2024 preferred)
- ✅ Matches your subsection topic

---

## Time Estimate

- Finding papers: 2-3 hours
- Reading abstracts/results: 4-5 hours
- Integrating into text: 2-3 hours
- **Total**: 8-11 hours

**Tip**: Work in 2-hour blocks, download 4-5 papers per session
