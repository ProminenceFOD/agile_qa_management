...
const menuItems = [
  {
    section: 'Overview',
    items: [
      { key: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, shortcut: 'Ctrl+1', hasNotification: false },
      { key: 'kanban', label: 'Kanban Board', icon: Trello, shortcut: 'Ctrl+2', hasNotification: false },
    ]
  },
  {
    section: 'Quality Management',
    items: [
      { key: 'validator', label: 'Stories', icon: FileText, shortcut: 'Ctrl+3', hasNotification: true, notificationKey: 'stories' },
      { key: 'tests', label: 'Test Cases', icon: TestTube, shortcut: 'Ctrl+6', hasNotification: true, notificationKey: 'tests' },
      { key: 'bugs', label: 'Bugs', icon: Bug, shortcut: 'Ctrl+7', hasNotification: true, notificationKey: 'bugs' },
      { key: 'risk', label: 'Risk Matrix', icon: Target, shortcut: 'Ctrl+4', hasNotification: false },
      { key: 'burndown', label: 'Burn-Down', icon: TrendingDown, shortcut: 'Ctrl+5', hasNotification: false },
    ]
  },
  {
    section: 'Analytics & Reports',
    items: [
      { key: 'charts', label: 'Analytics', icon: BarChart3, shortcut: 'Ctrl+8', hasNotification: false },
      { key: 'reports', label: 'Reports', icon: FileBarChart, shortcut: 'Ctrl+O', hasNotification: false },
      { key: 'testhistory', label: 'Test History', icon: History, shortcut: 'Ctrl+U', hasNotification: false },
      { key: 'team', label: 'Team Performance', icon: UsersIcon, shortcut: 'Ctrl+M', hasNotification: false },
    ]
  },
  {
    section: 'Planning & Workflow',
    items: [
      { key: 'sprints', label: 'Sprints', icon: Calendar, shortcut: 'Ctrl+9', hasNotification: false },
      { key: 'traceability', label: 'Traceability', icon: GitBranch, shortcut: 'Ctrl+G', hasNotification: false },
      { key: 'release', label: 'Release Readiness', icon: Rocket, shortcut: 'Ctrl+K', hasNotification: false },
      { key: 'recommendations', label: 'AI Recommend', icon: Brain, shortcut: 'Ctrl+I', hasNotification: false },
    ]
  },
  {
    section: 'Administration',
    items: [
      { key: 'audit', label: 'Audit Trail', icon: Shield, shortcut: 'Ctrl+A', hasNotification: false },
      { key: 'bulk', label: 'Bulk Operations', icon: Layers, shortcut: 'Ctrl+B', hasNotification: false },
      { key: 'data', label: 'Data Management', icon: Database, shortcut: 'Ctrl+E', hasNotification: false },
      { key: 'users', label: 'Users', icon: Users, shortcut: 'Ctrl+0', hasNotification: false },
    ]
  }
];

export function Sidebar({ activeTab, onTabChange, mobileOpen, onMobileToggle, bugCount = 0, storyCount = 0, testCount = 0, collapsed, onCollapsedChange }: SidebarProps) {
  ...
  const filteredSections = menuItems.map(section => ({
    ...
    items: section.items.filter(item =>
      item.label.toLowerCase().includes(searchQuery.toLowerCase()) && canAccessTab(currentUser?.role, item.key)
    )
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed left-0 top-0 h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 z-40
        ${collapsed ? 'w-16' : 'w-64'}
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              {!collapsed && (
                <div className="flex flex-col min-w-0">
                  <h2 className="text-lg font-semibold text-gray-900 dark:text-white leading-tight">AQMS</h2>
                  {currentUser?.organizationName && (
                    <p className="text-xs text-gray-600 dark:text-gray-400 truncate" title={currentUser.organizationName}>
                      {currentUser.organizationName}
                    </p>
                  )}
                </div>
            ...
            >
              <span className="text-xl font-light">{collapsed ? '»' : '«'}</span>
            </button>
          </div>
        </div>
                ...
                >
                  <button
                    onClick={() => {
                      ...
                      setTimeout(() => localStorage.removeItem('aqms_quick_create'), 100);
                    }}
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1">
                {favorites.map(favKey => {
                  const item = menuItems.flatMap(s => s.items).find(i => i.key === favKey);
                  if (!item) return null;
                  ...
                  return (
                    ...
                    >
                      ...
                      {count > 0 && !isActive && (
                        <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{count}</span>
                      )}
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1">
                {recentPages.slice(0, 3).map(recentKey => {
                  const item = menuItems.flatMap(s => s.items).find(i => i.key === recentKey);
                  if (!item) return null;
                  const Icon = item.icon;
                  const isActive = activeTab === item.key;
            ...
            <div key={idx} className="mb-6">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  ...
                  const count = item.hasNotification ? getNotificationCount(item.key) : 0;
                      ...
                      >
                        ...
                        {!collapsed && (
                          <>
                            ...
                            <span className="text-xs opacity-60">{item.shortcut.replace('Ctrl+', '⌘')}</span>
                          </>
                        )}
                        {collapsed && count > 0 && (
