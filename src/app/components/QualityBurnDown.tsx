...
export function QualityBurnDown() {
  ...
  const { data: stories, setData: setStories } = useSupabaseData<StoryItem[]>(
    'aqms_burndown_stories',
    defaultBurndownStories
  );
  ...
  const storiesRemaining =
    stateCounts['Testing'] +
    stateCounts['Bugs Found'] +
    stateCounts['Not Started'];
  ...
  const projectedRemainingDays =
    completionRate > 0 ? storiesRemaining / completionRate : Infinity;
  ...
  if (actualProgress >= idealProgress * 0.9) {
    ...
    healthIcon = '✅';
    healthMessage =
      'Sprint is progressing well. Current velocity suggests on-time completion.';
  } else if (
    actualProgress >= idealProgress * 0.7 &&
    projectedRemainingDays <= daysRemaining * 1.2
  ) {
    ...
    healthIcon = '⚠️';
    healthMessage =
      'Sprint is behind schedule but recoverable with focused effort.';
    ...
    healthIcon = '⏰';
    healthMessage =
      'Sprint is significantly behind. Consider descoping or extending timeline.';
  } else {
    ...
    healthIcon = '🚨';
    healthMessage =
      'Sprint is critically behind schedule. Immediate intervention required.';
  }

  const burnDownData = Array.from({ length: sprintDays + 1 }, (_, i) => {
    const day = i;
    const testedByDay = stories.filter(
      (s) => s.dayCompleted !== undefined && s.dayCompleted <= day
    ).length;
    const idealCompleted = (day / sprintDays) * totalStories;
    const projectedCompleted =
      day <= currentDay ? testedByDay : completionRate * day;
    ...
    return {
      ...
      remaining:
        stories.length - (day <= currentDay ? testedByDay : projectedCompleted),
    };
  });
      ...
      <div className="mb-8">
        <div>
          ...
          <p className="text-gray-600">
            Real-time trajectory tracking with four sprint health states (On
            Track, At Risk, Behind Schedule, Critical) and automated deadline
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <div className="font-semibold">
                  {storiesCompleted} / {totalStories} stories
                </div>
                <div className="text-xs opacity-75">
                  {Math.round((storiesCompleted / totalStories) * 100)}%
                  complete
                </div>
              </div>
              <div>
                ...
                <div className="font-semibold">
                  {completionRate.toFixed(1)} stories/day
                </div>
                <div className="text-xs opacity-75">
                  {currentDay > 0 ? 'Current pace' : 'Not started'}
                </div>
              </div>
              <div>
                <div className="opacity-75 mb-1">Projected Completion</div>
                <div className="font-semibold">
                  {Math.round(projectedCompletion)} stories
                </div>
                <div className="text-xs opacity-75">
                  {projectedCompletion >= totalStories
                    ? 'All stories'
                    : `${totalStories - Math.round(projectedCompletion)} at risk`}
                </div>
              </div>
              <div>
                ...
                <div className="text-xs opacity-75">
                  {projectedRemainingDays === Infinity
                    ? 'No progress yet'
                    : `Need ${projectedRemainingDays.toFixed(1)} days`}
                </div>
              </div>
            </div>
          </div>
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-center gap-2 flex-wrap">
                    ...
                    <button
                      ...
                      disabled={
                        story.state === 'Tested' || user?.role !== 'QA Engineer'
                      }
