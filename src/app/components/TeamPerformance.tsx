...
export function TeamPerformance() {
  ...
  const teamMetrics = useMemo(() => {
    return teamMembers.map((member) => {
      const memberStories = stories.filter(
        (s) =>
          s.assignedTo === member &&
          (selectedSprint === 'All Sprints' || s.sprint === selectedSprint)
      );
      ...
      const completedStories = memberStories.filter(
        (s) => s.status === 'Done'
      ).length;
      const totalPoints = memberStories.reduce(
      ...
      const avgExecutionTime =
        memberTests
          ...
          memberTests.filter((tc) => tc.executionTime).length || 0;

      const qualityScore = Math.round(
        testPassRate * 0.4 +
          (memberStories.length > 0
            ? (approvedStories / memberStories.length) * 100
            : 0) *
            0.4 +
          (memberBugs.length > 0
            ? (bugsResolved / memberBugs.length) * 100
            : 100) *
            0.2
      );
      ...
      return {
        ...
        approvalRate:
          memberStories.length > 0
            ? Math.round((approvedStories / memberStories.length) * 100)
            : 0,
        bugsCreated,
        ...
        bugResolutionRate:
          memberBugs.length > 0
            ? Math.round((bugsResolved / memberBugs.length) * 100)
            : 0,
        totalTests,
        passedTests,
        testPassRate,
  ...
  const teamTotals = useMemo(() => {
    return {
      ...
      completedStories: teamMetrics.reduce(
        (sum, m) => sum + m.completedStories,
        0
      ),
      ...
      avgQualityScore:
        teamMetrics.length > 0
          ? Math.round(
              teamMetrics.reduce((sum, m) => sum + m.qualityScore, 0) /
                teamMetrics.length
            )
          : 0,
      totalBugsResolved: teamMetrics.reduce(
        (sum, m) => sum + m.bugsResolved,
        0
      ),
  ...
  const performanceRankings = useMemo(() => {
    return {
      ...
      byQuality: [...teamMetrics].sort(
        (a, b) => b.qualityScore - a.qualityScore
      ),
      byTestPass: [...teamMetrics].sort(
        (a, b) => b.testPassRate - a.testPassRate
      ),
      byApproval: [...teamMetrics].sort(
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Individual and team productivity & quality metrics
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg mb-4 text-gray-900">
              Team Member Velocity Comparison
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  {performanceRankings.byVelocity
                    .slice(0, 3)
                    .map((member, index) => (
                      <div
                        key={member.name}
                        className="flex items-center gap-3 p-2 bg-indigo-50 rounded"
                      >
                    ...
                    .map((member, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-600">
                            {member.qualityScore}% quality score
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="space-y-6">
          ...
          {selectedMemberData ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="text-3xl mb-1">
                    {selectedMemberData.qualityScore}%
                  </div>
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                  ...
                  <div className="text-3xl mb-1">
                    {selectedMemberData.testPassRate}%
                  </div>
                  <div className="text-xs text-purple-600">
                    {selectedMemberData.passedTests}/
                    {selectedMemberData.totalTests}
                  </div>
                </div>
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                  ...
                  <div className="text-3xl mb-1">
                    {selectedMemberData.bugResolutionRate}%
                  </div>
                  <div className="text-xs text-orange-600">
                    {selectedMemberData.bugsResolved} Resolved
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6">
                <h3 className="text-lg mb-4 text-gray-900">
                  Performance Profile - {selectedMember}
                </h3>
                <ResponsiveContainer width="100%" height={400}>
                  <RadarChart
                    data={[
                      {
                        metric: 'Velocity',
                        value: Math.min(selectedMemberData.velocity, 100),
                      },
                      {
                        metric: 'Quality',
                        value: selectedMemberData.qualityScore,
                      },
                      {
                        metric: 'Test Pass',
                        value: selectedMemberData.testPassRate,
                      },
                      {
                        metric: 'Approval',
                        value: selectedMemberData.approvalRate,
                      },
                      {
                        metric: 'Bug Resolution',
                        value: selectedMemberData.bugResolutionRate,
                      },
                    ]}
                  >
                    <PolarGrid />
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {selectedMemberData.totalStories}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Completed</span>
                      <span className="font-medium text-green-600">
                        {selectedMemberData.completedStories}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Approved</span>
                      <span className="font-medium text-indigo-600">
                        {selectedMemberData.approvedStories}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Approval Rate</span>
                      <span className="font-medium">
                        {selectedMemberData.approvalRate}%
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        Bugs Created (Reported)
                      </span>
                      <span className="font-medium">
                        {selectedMemberData.bugsCreated}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Bugs Assigned</span>
                      <span className="font-medium">
                        {selectedMemberData.bugsAssigned}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Bugs Resolved</span>
                      <span className="font-medium text-green-600">
                        {selectedMemberData.bugsResolved}
                      </span>
                    </div>
                    <div className="flex justify-between">
              ...
              <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
                ...
                <ul className="text-sm text-gray-700 space-y-2">
                  <li>
                    • <strong>Velocity:</strong> {selectedMemberData.velocity}{' '}
                    points{' '}
                    {selectedMemberData.velocity >=
                    teamTotals.totalVelocity / teamMetrics.length
                      ? '(Above team average ✓)'
                      : '(Below team average)'}
                  </li>
                  <li>
                    ...
                    {selectedMemberData.qualityScore}%{' '}
                    {selectedMemberData.qualityScore >=
                    teamTotals.avgQualityScore
                      ? '(Above team average ✓)'
                      : '(Below team average)'}
                  </li>
                  <li>
                    • <strong>Test Performance:</strong>{' '}
                    {selectedMemberData.testPassRate}% pass rate{' '}
                    {selectedMemberData.testPassRate >= 85
                      ? '(Excellent ✓)'
                      : '(Needs improvement)'}
                  </li>
                  {selectedMemberData.avgExecutionTime > 0 && (
                    <li>
                      • <strong>Avg Test Execution:</strong>{' '}
                      {selectedMemberData.avgExecutionTime}ms per test
                    </li>
                  )}
                </ul>
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-12 text-center">
              <p className="text-gray-600">
                Please select a team member to view their performance metrics.
              </p>
            </div>
          )}
        </div>
                        ...
                        <span
                          className={`inline-flex px-3 py-1 rounded-full text-sm ${
                            member.qualityScore >= 90
                              ...
                              : member.qualityScore >= 75
                                ? 'bg-indigo-100 text-indigo-800'
                                : member.qualityScore >= 60
                          ...
                          <div className="w-16 bg-gray-200 rounded-full h-2">
                            <div
                              className={`h-2 rounded-full ${
                                member.testPassRate >= 90
                                  ? 'bg-green-500'
                                  : member.testPassRate >= 75
                                    ? 'bg-indigo-500'
                                    : 'bg-yellow-500'
                              }`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
              <div className="text-2xl mb-1">
                {
                  teamMetrics.filter(
                    (m) => m.qualityScore >= 75 && m.qualityScore < 90
                  ).length
                }
              </div>
                  ...
                  {performanceRankings.byVelocity.map((member) => (
                    <tr key={member.name} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center text-sm text-gray-600">
                        {member.avgExecutionTime > 0
                          ? `${member.avgExecutionTime}ms`
                          : 'N/A'}
                      </td>
                    </tr>
                  ))}
