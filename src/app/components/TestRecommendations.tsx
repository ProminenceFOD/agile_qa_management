...
export function TestRecommendations({ onNavigate }: TestRecommendationsProps) {
  ...
  const recommendations = useMemo(() => {
    ...
    // Helper to check if a test case with the same parsed title is already created for the story/module
    const testExists = (title: string, storyId?: string, moduleId?: string) => {
      const getActualTitle = (t: string) => {
        const [_, ...parts] = t.split(':');
        return parts.length > 0 ? parts.join(':').trim() : t;
      };
      ...
      return testCases.some((tc) => {
        ...
        if (storyId && tc.linkedStory === storyId) return true;
        if (moduleId && tc.moduleId === moduleId) return true;
        return false;
      });
    };
