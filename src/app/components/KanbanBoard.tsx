...
function DraggableCard({
  ...
  const getQualityStatus = () => {
    ...
    const failedTests = linkedTestCases.filter(
      (tc) => tc.status === 'Fail'
    ).length;
    const blockedTests = linkedTestCases.filter(
      (tc) => tc.status === 'Blocked'
    ).length;
    const totalTests = linkedTestCases.length;
    const passedTests = linkedTestCases.filter(
      (tc) => tc.status === 'Pass'
    ).length;
    const noTests =
      totalTests === 0 &&
      (story.status === 'In Testing' || story.status === 'Bugs Found');
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        if (
          !isDragging &&
          !(e.target as HTMLElement).closest('button, select, input, a')
        ) {
          onView(item);
        }
      }}
    ...
    >
      ...
      {qualityStatus && (
        <div
          className={`${qualityStatus.color} ${qualityStatus.textColor} px-2 py-1 text-[10px] font-semibold flex items-center justify-between`}
        >
      ...
      <div className="p-3">
        ...
        <h4 className="text-xs font-semibold text-gray-900 mb-1.5 line-clamp-2 leading-tight">
          {item.title}
        </h4>
              ...
              <div className="space-y-1 mt-2">
                {linkedTestCases.map((tc) => (
                  ...
                  >
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs ml-2 flex-shrink-0 ${getTestStatusColor(tc.status)}`}
                    >
                      {tc.status}
                    </span>
                  </div>
  ...
  return (
    <div ref={drop} className="flex-1 min-w-[280px] max-w-[400px]">
      ...
      >
        ...
        <div
          className={`rounded-t-lg border-2 ${color} p-3 bg-white sticky top-0 z-20 shadow-sm`}
        >
...
export function KanbanBoard() {
  const { user } = useAuth();
  const [filterType, setFilterType] = useState<'all' | 'stories' | 'bugs'>(
    'all'
  );
  const [filterDeveloper, setFilterDeveloper] = useState<string>('');
  const [filterTester, setFilterTester] = useState<string>('');
  const [filterTag, setFilterTag] = useState<string>('');
  const [viewMode, setViewMode] = useState<'board' | 'storyView' | 'bugView'>(
    'board'
  );
  ...
  const getInitialBugs = (): Bug[] => {
    ...
    return [
      ...
      },
      {
        ...
        description:
          'Payment processing fails when user input contains special characters',
        severity: 'Critical',
        status: 'In Progress',
        linkedStory: 'US-102',
        ...
        steps: [
          'Enter card with name containing special chars',
          'Submit payment',
          'Observe failure',
        ],
  ...
  } = useSupabaseData<Story[]>('aqms_stories', getInitialStories());
  const {
    data: bugs,
    setData: setBugs,
    loading: bugsLoading,
  } = useSupabaseData<Bug[]>('aqms_bugs', getInitialBugs());
  ...
  // We need to group items by columns based on settings
  ...
  const filterByAssignment = (item: Story | Bug) => {
    ...
    if (filterTester) {
      const itemTester =
        'assignedTester' in item ? item.assignedTester : undefined;
  ...
  const columns: {
      ...
      items: [
        ...stories.filter(
          (s) =>
            s.status === 'Backlog' &&
            isApprovedStory(s) &&
            filterByAssignment(s)
        ),
        ...(filterType !== 'stories'
          ? bugs.filter((b) => b.status === 'Open' && filterByAssignment(b))
          : []),
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        ...(filterType !== 'stories'
          ? bugs.filter(
              (b) => b.status === 'In Progress' && filterByAssignment(b)
            )
          : []),
      ],
    },
    ...
    },
    {
      ...
      items: [
        ...stories.filter(
          (s) =>
            s.status === 'Done' && isApprovedStory(s) && filterByAssignment(s)
        ),
        ...(filterType !== 'stories'
          ? bugs.filter(
              (b) =>
  ...
  const handleDrop = (item: Story | Bug, newColumnStatus: string) => {
    if (isStory(item)) {
      ...
      else if (newColumnStatus === 'In Development')
      ...
      setStories(
        stories.map((s) => (s.id === item.id ? { ...s, status: newStatus } : s))
      );
    } else {
      ...
      setBugs(
        bugs.map((b) => (b.id === item.id ? { ...b, status: newStatus } : b))
      );
    }
  };
  ...
  const handleAssignDeveloper = (developer: string) => {
    if (selectedItem && isStory(selectedItem)) {
      ...
      setStories(
        stories.map((s) => (s.id === selectedItem.id ? updatedStory : s))
      );
  ...
  const handleAssignTester = (tester: string) => {
    if (selectedItem && isStory(selectedItem)) {
      const updatedStory = {
        ...selectedItem,
        assignedTester: tester || undefined,
      };
      setStories(
        stories.map((s) => (s.id === selectedItem.id ? updatedStory : s))
      );
  ...
  const handleCardAssignDeveloper = (itemId: string, developer: string) => {
    ...
    if (story) {
      setStories(
        stories.map((s) =>
          s.id === itemId
            ? { ...s, assignedDeveloper: developer || undefined }
            : s
        )
      );
    } else if (bug) {
        ...
        bugs.map((b) =>
          b.id === itemId
            ? {
                ...b,
                assignedDeveloper: developer || undefined,
                assignedTo: developer || undefined,
              }
            : b
        )
      );
  ...
  const handleLinkBug = (bugId: string) => {
    if (selectedItem && isStory(selectedItem)) {
      ...
      setBugs(
        bugs.map((b) =>
          b.id === bugId ? { ...b, linkedStory: selectedItem.id } : b
        )
      );
    }
  };
  ...
  const handleUnlinkBug = (bugId: string) => {
    if (selectedItem && isStory(selectedItem)) {
      const updatedStory = {
        ...selectedItem,
        linkedBugs: (selectedItem.linkedBugs || []).filter(
          (id) => id !== bugId
        ),
      };
      setStories(
      ...
      setBugs(
        bugs.map((b) => (b.id === bugId ? { ...b, linkedStory: undefined } : b))
      );
    }
  };
  ...
  const allSprints = Array.from(
    new Set(['Backlog', ...stories.map((s) => s.sprint).filter(Boolean)])
  ).sort();
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="w-full h-full p-4 md:p-6">
        <div className="mb-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700">
                    No stories or bugs found. The database may not be
                    accessible. Default data should load automatically.
                  </p>
                </div>
              </div>
            </div>
          ...
          {stories.length > 0 &&
            stories.filter((s) => s.qaSignOff && s.pmApproval).length === 0 && (
              <div className="mb-4 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-700">
                      {stories.length}{' '}
                      {stories.length === 1 ? 'story' : 'stories'} found, but
                      none have both QA sign-off and PM approval. Stories need
                      both approvals to appear on the Kanban board.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
            <div>
              ...
              <p className="text-gray-600 text-sm md:text-base">
                ...
                approved stories (of {stories.length} total), {bugs.length} bugs
              </p>
            </div>
            <div className="relative">
              <button
          ...
          <div className="mb-6 bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-200 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-600 mt-3 italic">
              Cards show quality states in real-time, exposing issues like bugs,
              blocked tests, and coverage gaps before sprint deadlines.
            </p>
          </div>
            ...
            {(filterDeveloper ||
              ...
              <div className="flex items-center justify-between mt-4 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                <div className="text-sm text-gray-700">
                  ...
                  <div className="mt-2">
                    Showing{' '}
                    {columns.reduce((sum, col) => sum + col.items.length, 0)} of{' '}
                    {stories.filter((s) => s.qaSignOff && s.pmApproval).length +
                      bugs.length}{' '}
                    available items
                  </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
                <div className="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800 rounded-t-lg flex-shrink-0">
                  <h2 className="text-xl md:text-2xl text-gray-900 dark:text-white font-semibold">
                    {editingItem && isStory(editingItem)
                      ? 'Edit User Story'
                  ...
                  onSubmit={(e) => {
                    ...
                    if (editingItem && isStory(editingItem)) {
                      ...
                      const updatedStory: Story = {
                        ...
                        storyPoints:
                          parseInt(formData.get('points') as string) || 0,
                      ...
                      setStories(
                        stories.map((s) =>
                          s.id === editingItem.id ? updatedStory : s
                        )
                      );
                      setSelectedItem(updatedStory);
                      setNotification({
                  ...
                  onSubmit={(e) => {
                    ...
                    if (editingItem && !isStory(editingItem)) {
                      ...
                      const updatedBug: Bug = {
                        ...
                        linkedStory:
                          (formData.get('linkedStory') as string) || undefined,
                      ...
                      setBugs(
                        bugs.map((b) =>
                          b.id === editingItem.id ? updatedBug : b
                        )
                      );
                      setSelectedItem(updatedBug);
                      setNotification({
                        ...
                        {!showNewStoryFromBug ? (
                          <div className="flex gap-2">
                            ...
                            >
                              ...
                              {stories
                                .filter((s) => s.qaSignOff && s.pmApproval)
                                .map((story) => (
                          ...
                          <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900 rounded-lg p-3">
                            <p className="text-sm text-indigo-800 dark:text-indigo-350 mb-2">
                              To create a new story, please close this form and
                              use the "+ Create New" button, then link the bug
                              from the story view.
                            </p>
                            <button
                              type="button"
                ...
                <form
                  onSubmit={async (e) => {
                    ...
                    const newTestCase = {
                      ...
                      preconditions:
                        (formData.get('preconditions') as string) || undefined,
          ...
          <TestCaseExecute
            ...
            onCreateBug={(testCase, notes) => {
              ...
              setNotification({
                ...
                title: 'Create Bug',
                message:
                  'Bug creation from Kanban board will be available soon. Please use the Test Cases tab (Ctrl+6) to create bugs from failed tests.',
                type: 'info',
              });
            }}
          />
              ...
              const updatedTestCases = existingTestCases.map((tc: any) =>
                tc.id === editingTestCase.id
                  ? {
                      ...
                      lastRun: editingTestCase.lastRun,
                      executionTime: editingTestCase.executionTime,
                    }
                  : tc
              );
