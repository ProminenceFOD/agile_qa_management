...
export function TraceabilityMatrix({
  ...
  const { data: stories } = useSupabaseData<Story[]>(
    'aqms_stories',
    defaultStories
  );
  ...
  const traceabilityData = useMemo(() => {
    return stories.map((story) => {
      const linkedTests = testCases.filter(
        (tc) => tc.linkedStory === story.id && !tc.isDraft
      );
      const linkedBugs = bugs.filter((bug) => bug.linkedStory === story.id);

      const totalTests = linkedTests.length;
      const passedTests = linkedTests.filter(
        (tc) => tc.status === 'Pass'
      ).length;
      const failedTests = linkedTests.filter(
        (tc) => tc.status === 'Fail'
      ).length;
      const blockedTests = linkedTests.filter(
      ...
      const activeBugs = linkedBugs.filter(
  ...
  const overallMetrics = useMemo(() => {
    ...
    const storiesWithTests = traceabilityData.filter(
      (item) => item.metrics.totalTests > 0
    ).length;
    const storiesWithGaps = traceabilityData.filter(
      (item) => item.metrics.hasGaps
    ).length;
    const storiesFullyCovered = traceabilityData.filter(
      (item) =>
        item.metrics.totalTests > 0 && item.metrics.coverageScore === 100
    ).length;
    ...
    const passedTestsCount = totalActiveTestCases.filter(
      (tc) => tc.status === 'Pass'
    ).length;
    const notRunTestsCount = totalActiveTestCases.filter(
    ...
    <div className="w-full max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-650 dark:text-gray-400">
          End-to-end coverage from requirements to tests to defects
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900/50 rounded-lg p-4">
          <div className="text-2xl font-bold mb-1 text-indigo-700 dark:text-indigo-400">
            {overallMetrics.testCoverageRate}%
          </div>
          <div className="text-gray-650 dark:text-gray-400 text-sm">
            Stories with Test Coverage
          </div>
          <div className="text-xs text-indigo-655 dark:text-indigo-400/80 mt-1">
            {overallMetrics.storiesWithTests}/{overallMetrics.totalStories}{' '}
            Stories
          </div>
        </div>
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-lg p-4">
          <div className="text-2xl font-bold mb-1 text-red-700 dark:text-red-400">
            {overallMetrics.storiesWithGaps}
          </div>
          <div className="text-gray-650 dark:text-gray-400 text-sm">
            Coverage Gaps
          </div>
        ...
        <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-900/50 rounded-lg p-4">
          <div className="text-2xl font-bold mb-1 text-purple-700 dark:text-purple-400">
            {overallMetrics.totalTestCases}
          </div>
          <div className="text-gray-650 dark:text-gray-400 text-sm">
            Total Test Cases
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  <th className="px-6 py-4 text-left text-gray-700 dark:text-gray-300 font-semibold w-32">
                    Story ID
                  </th>
                  <th className="px-6 py-4 text-left text-gray-700 dark:text-gray-300 font-semibold">
                    Story Title
                  </th>
                  <th className="px-6 py-4 text-center text-gray-700 dark:text-gray-300 font-semibold">
                    Test Cases
                  </th>
                  <th className="px-6 py-4 text-center text-gray-700 dark:text-gray-300 font-semibold">
                    Pass Rate
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Status: {item.story.status} | QA:{' '}
                        {item.story.qaSignOff ? '✓' : '✗'} | PM:{' '}
                        {item.story.pmApproval ? '✓' : '✗'}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="text-sm text-gray-900 dark:text-white font-medium">
                        {item.metrics.totalTests} total
                      </div>
                      <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        {item.metrics.passedTests}P / {item.metrics.failedTests}
                        F / {item.metrics.blockedTests}B
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(item.metrics.hasGaps)}`}
                      >
      ...
      {viewMode === 'coverage' && (
        <div className="space-y-6">
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {[
                {
                  label: '100% Covered',
                  count: traceabilityData.filter(
                    (i) => i.metrics.coverageScore === 100
                  ).length,
                  color: 'green',
                },
                {
                  ...
                  count: traceabilityData.filter(
                    (i) =>
                      i.metrics.coverageScore >= 75 &&
                      i.metrics.coverageScore < 100
                  ).length,
                  color: 'blue',
                },
                {
                ...
                <div
                  key={item.label}
                  className={`border rounded-lg p-4 transition-all duration-200 hover:shadow-sm ${getDistributionColorStyles(item.color)}`}
                >
                  <div
                    className={`text-3xl font-bold mb-1 ${getDistributionTextColorStyles(item.color)}`}
                  >
                    {item.count}
                  </div>
                  <div className="text-sm text-gray-650 dark:text-gray-300 font-medium">
                    {item.label}
                  </div>
                </div>
              ))}
          ...
          <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900/50 rounded-lg p-4">
            <h4 className="text-indigo-900 dark:text-indigo-300 font-semibold mb-2 flex items-center gap-1.5">
              📊 Coverage Insights
            </h4>
            <ul className="text-sm text-indigo-950 dark:text-indigo-400 space-y-2 list-none pl-0">
              <li>
                • <strong>{overallMetrics.storiesWithTests}</strong> out of{' '}
                <strong>{overallMetrics.totalStories}</strong> stories have test
                coverage ({overallMetrics.testCoverageRate}%)
              </li>
              <li>
                •{' '}
                <strong>
                  {
                    traceabilityData.filter((i) => i.metrics.totalTests === 0)
                      .length
                  }
                </strong>{' '}
                stories have zero test cases - priority for test creation
              </li>
              <li>
                •{' '}
                <strong>
                  {
                    traceabilityData.filter((i) => i.metrics.failedTests > 0)
                      .length
                  }
                </strong>{' '}
                stories have failing tests - require immediate attention
              </li>
              <li>
                • Average{' '}
                <strong>
                  {testCases.length > 0
                    ? (
                        testCases.length /
                        Math.max(overallMetrics.storiesWithTests, 1)
                      ).toFixed(1)
                    : 0}
                </strong>{' '}
        ...
        <div className="space-y-4">
          <div className="bg-red-50 dark:bg-red-950/20 border-l-4 border-red-500 p-4 rounded-r-lg">
            ...
            <p className="text-sm text-red-750 dark:text-red-300">
              {traceabilityData.filter((i) => i.metrics.hasGaps).length} stories
              have coverage gaps that need to be addressed
            </p>
          </div>

          {traceabilityData
            .filter((i) => i.metrics.hasGaps)
            .map((item) => (
              <div
                key={item.story.id}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="bg-gray-50 dark:bg-gray-900/30 rounded p-3">
                    ...
                    <div className="text-lg font-semibold text-gray-900 dark:text-white">
                      {item.metrics.totalTests} tests (
                      {item.metrics.coverageScore}% pass rate)
                    </div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-900/30 rounded p-3">
                    <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                    ...
                    <div className="text-lg font-semibold text-red-650 dark:text-red-400">
                      {item.metrics.failedTests + item.metrics.blockedTests}{' '}
                      issues
                    </div>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-900/30 rounded p-3">
                    ...
                    <div className="text-lg font-semibold text-red-650 dark:text-red-400">
                      {item.metrics.activeBugs} ({item.metrics.criticalBugs}{' '}
                      critical)
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-yellow-50 dark:bg-yellow-950/15 border border-yellow-250 dark:border-yellow-900/30 rounded p-3">
                  ...
                  <ul className="text-sm text-yellow-950 dark:text-yellow-400/90 space-y-1 list-none pl-0">
                    {item.metrics.totalTests === 0 && (
                      <li>
                        ⚠️ No test cases linked - create comprehensive test
                        coverage
                      </li>
                    )}
                    {item.metrics.failedTests > 0 && (
                      <li>
                        ❌ {item.metrics.failedTests} failing tests -
                        investigate and resolve failures
                      </li>
                    )}
                    {item.metrics.blockedTests > 0 && (
                      <li>
                        🚫 {item.metrics.blockedTests} blocked tests - remove
                        blockers to continue testing
                      </li>
                    )}
                    {item.metrics.criticalBugs > 0 && (
                      <li>
                        🐛 {item.metrics.criticalBugs} critical bugs -
                        prioritize bug resolution before release
                      </li>
                    )}
                    {item.metrics.activeBugs > 0 &&
                      item.metrics.criticalBugs === 0 && (
                        <li>
                          🐛 {item.metrics.activeBugs} active bugs - address
          ...
          {traceabilityData.filter((i) => i.metrics.hasGaps).length === 0 && (
            <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900/50 rounded-lg p-8 text-center">
              ...
              <p className="text-gray-655 dark:text-gray-400">
                All stories have adequate test coverage with no failing tests or
                critical bugs.
              </p>
            </div>
          )}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-lg text-indigo-600 dark:text-indigo-400">
                      {selectedItem.story.id}
                    </span>
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-semibold ${
                        selectedItem.story.priority === 'Critical'
                          ? 'bg-red-100 text-red-800 dark:bg-red-950/30 dark:text-red-300'
                          : selectedItem.story.priority === 'High'
                            ? 'bg-orange-100 text-orange-800 dark:bg-orange-950/30 dark:text-orange-300'
                            : selectedItem.story.priority === 'Medium'
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      | {selectedItem.story.sprint || 'No Sprint'}
                    </span>
                  </div>
              ...
              <div className="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div>
                  ...
                  <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-sm bg-gray-50 dark:bg-gray-900/30 p-4 rounded-lg border border-gray-100 dark:border-gray-800">
                    {selectedItem.story.description ||
                ...
                <div>
                  ...
                  <div className="text-gray-750 dark:text-gray-300 leading-relaxed text-sm bg-gray-50 dark:bg-gray-900/30 p-4 rounded-lg border border-gray-100 dark:border-gray-800 whitespace-pre-line font-mono">
                    {selectedItem.story.criteriaDetails ||
                      'No acceptance criteria details defined.'}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span
                        className={`w-2 h-2 rounded-full ${selectedItem.story.qaSignOff ? 'bg-green-500' : 'bg-red-500'}`}
                      ></span>
                      <span className="font-semibold text-sm text-gray-900 dark:text-white">
                        {selectedItem.story.qaSignOff
                          ? '✓ Signed Off'
                          : '✗ Pending'}
                      </span>
                    </div>
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span
                        className={`w-2 h-2 rounded-full ${selectedItem.story.pmApproval ? 'bg-green-500' : 'bg-red-500'}`}
                      ></span>
                      <span className="font-semibold text-sm text-gray-900 dark:text-white">
                        {selectedItem.story.pmApproval
                          ? '✓ Approved'
                          : '✗ Pending'}
                      </span>
                    </div>
                ...
                <div>
                  ...
                  {selectedItem.linkedTests.length > 0 ? (
                        ...
                        <tbody className="divide-y divide-gray-150 dark:divide-gray-750">
                          {selectedItem.linkedTests.map((tc: TestCase) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-2 text-gray-750 dark:text-gray-300 font-medium">
                                {tc.title}
                              </td>
                                ...
                                <span
                                  className={`inline-flex px-2 py-0.5 rounded text-xs font-semibold ${
                                    tc.status === 'Pass'
                                      ? 'bg-green-100 text-green-800 dark:bg-green-950/20 dark:text-green-300'
                                      : tc.status === 'Fail'
                                        ? 'bg-red-100 text-red-800 dark:bg-red-950/20 dark:text-red-300 animate-pulse'
                                        : tc.status === 'Blocked'
                    ...
                    <div className="bg-orange-50 dark:bg-orange-950/15 border border-orange-200 dark:border-orange-900/30 rounded-lg p-4 text-sm text-orange-850 dark:text-orange-400 flex items-center gap-2">
                      <span>⚠️</span>
                      <span>
                        No test cases linked to this story. We recommend
                        creating test coverages before dev sign-off.
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {selectedItem.linkedBugs.length > 0 ? (
                    ...
                    <div className="bg-green-50 dark:bg-green-950/15 border border-green-200 dark:border-green-900/30 rounded-lg p-4 text-sm text-green-850 dark:text-green-400 flex items-center gap-2">
                      <span>✅</span>
                      <span>
                        No defects linked to this story. Quality standards fully
                        satisfied.
                      </span>
                    </div>
                  )}
