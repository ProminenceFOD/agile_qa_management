...
export function SprintForm({
  ...
  const [formData, setFormData] = useState({
    ...
    status:
      sprint?.status || ('Planning' as 'Planning' | 'Active' | 'Completed'),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newSprint: Omit<
      Sprint,
      'id' | 'committed' | 'completed' | 'stories'
    > = {
      name: formData.name,
          ...
          >
            <div className="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-900/40 rounded-lg p-3 text-sm text-indigo-800 dark:text-indigo-200">
                <strong>Note:</strong> Stories can be added to the sprint after
                creation. Committed and completed points will be tracked as
                stories are added and updated.
              </div>
            </div>
