...
import { Edit3, AlertTriangle, Check, X, Shield, Clock, FileText } from 'lucide-react';
...
import {
  hasPermission, isAdmin, getPermissionsForRole, canAccessTab,
  DEFAULT_ROLE_PERMISSIONS, getCurrentRolePermissions, overrideRolePermissions,
  TAB_PERMISSION_MAP, PERMISSION_TEMPLATES, ALL_PERMISSIONS,
  type Permission, type AnyRole, type AuditLogEntry, type UserPermissionOverride,
  getUserOverride, grantUserPermission, revokeUserPermission, clearUserOverrides,
  loadUserOverrides, saveUserOverrides, getAuditLogs, logAuditEntry, applyTemplateToRole, applyTemplateToUser,
} from '../utils/permissions';
import { getData as getStorageData, setData as setStorageData } from '../utils/supabaseStorage';
...
interface User {
  ...
  name: string;
  email: string;
  role: 'Administrator' | 'QA Engineer' | 'Product Manager' | 'Scrum Master' | 'Developer' | 'Tester';
  title?: string;
...
export function UserManagement() {
  ...
  const [templateTarget, setTemplateTarget] = useState<{type: 'role' | 'user', id: string, name: string} | null>(null);
  ...
  const { data: users, setData: setUsers, loading: usersLoading } = useSupabaseData<User[]>('aqms_users', getInitialUsers());
  ...
  const canDeactivateUsers = hasPermission(currentUser?.role, 'users:deactivate');
  ...
  const handleAddUserDirectly = () => {
    ...
    const existingUser = users.find(u => u.email.toLowerCase() === addUserEmail.toLowerCase());
    if (existingUser) {
  ...
  const handleSendInvite = () => {
    ...
    showSuccess(`Invitation sent to ${inviteEmail}! (Note: Email service not configured in prototype)`);
  };
  ...
  const handleDeactivateUser = (userId: string) => {
    showConfirm(
      ...
      () => {
        setUsers(
          users.map(u => (u.id === userId ? { ...u, status: 'Inactive' as const } : u))
        );
      },
      'Deactivate User',
      'Deactivate',
  ...
  const handleToggleQASignOff = (userId: string) => {
    ...
    if (newValue && !user.title) {
      showAlert('Please set a title for this user first (e.g., "Head of QA", "Senior QA Lead")');
      return;
    }

    setUsers(users.map(u =>
      u.id === userId
        ? { ...u, canSignOffQA: newValue }
        : u
    ));
  ...
  const handleTogglePMSignOff = (userId: string) => {
    ...
    if (newValue && !user.title) {
      showAlert('Please set a title for this user first (e.g., "Senior Product Manager", "Product Lead")');
      return;
    }

    setUsers(users.map(u =>
      u.id === userId
        ? { ...u, canSignOffPM: newValue }
        : u
    ));
    // useSupabaseData handles saving automatically
  };
  ...
  const handleSaveTitle = () => {
    ...
    setUsers(users.map(u =>
      u.id === editingUserId ? { ...u, title: editTitleValue.trim() || undefined } : u
    ));
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <div className="mb-6 bg-purple-50 border border-purple-200 rounded-lg p-4">
        <h3 className="text-sm font-medium text-purple-900 mb-2">🔐 Sign-Off Authority</h3>
        <p className="text-sm text-purple-800 mb-3">
          QA Engineers and Product Managers with sign-off authority can approve stories.
          Click authority buttons to grant/revoke permissions. Only designated reviewers can provide approvals.
        </p>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center text-sm text-gray-700">Authority</th>
                  <th className="px-6 py-4 text-center text-sm text-gray-700">Status</th>
                  <th className="px-6 py-4 text-center text-sm text-gray-700">Stories</th>
                  <th className="px-6 py-4 text-center text-sm text-gray-700">Bugs</th>
                  <th className="px-6 py-4 text-left text-sm text-gray-700">Last Active</th>
                  ...
                  <tr key={user.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-700">
                          {user.title || <span className="text-gray-400 italic">No title</span>}
                        </span>
                        <button
                    ...
                    <td className="px-6 py-4 text-center">
                      {user.role === 'QA Engineer' ? (
                        ...
                        >
                          {user.canSignOffQA ? '✓ QA Sign-Off' : 'No QA Authority'}
                        </button>
                        ...
                        >
                          {user.canSignOffPM ? '✓ PM Approval' : 'No PM Authority'}
                        </button>
                      ) : (
      ...
      {activeTab === 'invitations' && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm text-gray-700">Role</th>
                <th className="px-6 py-4 text-left text-sm text-gray-700">Sent By</th>
                <th className="px-6 py-4 text-center text-sm text-gray-700">Status</th>
                <th className="px-6 py-4 text-left text-sm text-gray-700">Sent Date</th>
                <th className="px-6 py-4 text-left text-sm text-gray-700">Expires</th>
                <th className="px-6 py-4 text-center text-sm text-gray-700">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {invitations.map(invitation => (
                <tr key={invitation.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{invitation.sentBy}</td>
                  <td className="px-6 py-4 text-center">
                    <span
          ...
          {invitations.length === 0 && (
            <div className="text-center py-12 text-gray-500">No invitations sent yet</div>
          )}
        </div>
      )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col overflow-hidden">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800 rounded-t-lg flex-shrink-0">
                <h2 className="text-2xl text-gray-900 dark:text-white font-semibold">Invite Team Member</h2>
                <button
                  onClick={() => setShowInviteModal(false)}
              ...
              <div className="p-6 space-y-4 flex-grow overflow-y-auto custom-scrollbar">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-950/30 border border-yellow-200 dark:border-yellow-900 rounded-lg p-3 text-sm text-yellow-800 dark:text-yellow-350">
                  <strong>⚠️ Prototype Mode:</strong> Email service is not configured. The invitation
                  will be recorded in the system but no actual email will be sent. For immediate user
                  access, use <strong>"Add User"</strong> instead.
                </div>
              </div>
      ...
      {showAddUserModal && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col overflow-hidden">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800 rounded-t-lg flex-shrink-0">
                <h2 className="text-2xl text-gray-900 dark:text-white font-semibold">Add User Directly</h2>
                <button
                  onClick={() => setShowAddUserModal(false)}
                  className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 text-2xl"
                >
              ...
              <div className="p-6 space-y-4 flex-grow overflow-y-auto custom-scrollbar">
                <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 rounded-lg p-3 text-sm text-green-800 dark:text-green-300">
                  <strong>✓ Direct Add:</strong> User will be created immediately with Active status.
                  They can log in straight away using their email and the default password: <strong>password123</strong>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={addUserRole}
                    onChange={e => setAddUserRole(e.target.value as User['role'])}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  >
                    <option value="Developer">Developer</option>
                    <option value="Tester">Tester</option>
...
const MATRIX_ROLES: AnyRole[] = [
  'Administrator', 'QA Engineer', 'Product Manager', 'Scrum Master', 'Developer', 'Tester',
];
...
const PAGE_VIEWS: { label: string; tab: string; requiredPermission: Permission }[] = Object.entries(TAB_PERMISSION_MAP).map(
  ([tab, perm]) => ({
    tab,
    label: {
      dashboard:     'Dashboard',
...
const ACTION_GROUPS: { group: string; items: { label: string; permission: Permission }[] }[] = [
  { group: 'Stories', items: [
...
function buildSets(map: Record<string, Permission[]>): Record<string, Set<Permission>> {
...
function PermissionsMatrix({ currentUser }: PermissionsMatrixProps) {
  ...
  const pageAccessible = (role: AnyRole, requiredPermission: Permission): boolean =>
    role === 'Administrator' || (permSets[role]?.has(requiredPermission) ?? false);
  ...
  const handleReset = () => {
    setPermSets(buildSets(DEFAULT_ROLE_PERMISSIONS as Record<string, Permission[]>));
    setDirty(true);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2">
        Toggle checkboxes to grant or revoke permissions per role. <strong>Administrator</strong> always retains full access.
        Use the <strong>Select All / Deselect All</strong> buttons in column headers for bulk operations.
        Click <strong>Save Changes</strong> to apply immediately.
      </div>
      ...
      {section === 'pages' && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 min-w-[200px]">Page</th>
                <th className="px-3 py-2 text-xs text-gray-500 font-normal border-l border-gray-100">Required permission</th>
                {MATRIX_ROLES.map(role => (
                  <th key={role} className="px-3 py-3 text-center border-l border-gray-100 min-w-[110px]">
                    <div className="space-y-1">
                      <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${ROLE_COLORS[role]}`}>
                        {role}
                      </span>
                      {role !== 'Administrator' && (
            ...
            <tbody className="divide-y divide-gray-100">
              {PAGE_VIEWS.map(({ label, tab, requiredPermission }) => (
                <tr key={tab} className="hover:bg-gray-50">
                  <td className="px-4 py-2 text-gray-800 font-medium">{label}</td>
                  <td className="px-3 py-2 border-l border-gray-100">
                    <span className="text-xs font-mono text-gray-400">{requiredPermission}</span>
                  </td>
                  {MATRIX_ROLES.map(role => {
                    const isAdmin = role === 'Administrator';
                    const checked = isAdmin || hasPerm(role, requiredPermission);
                    return (
                      <td key={role} className="px-3 py-2 text-center border-l border-gray-100">
                        <input
                          type="checkbox"
                          checked={checked}
                          disabled={isAdmin}
                          ...
                          title={isAdmin ? 'Administrator always has full access' : `${checked ? 'Revoke' : 'Grant'} access to ${label} for ${role}`}
                        />
                      </td>
                    );
                  })}
      ...
      {section === 'actions' && (
        <div className="space-y-4">
          {ACTION_GROUPS.map(({ group, items }) => (
            <div key={group} className="bg-white rounded-lg border border-gray-200 overflow-x-auto">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="px-4 py-2 text-left text-xs font-semibold text-gray-600 min-w-[200px]">Action</th>
                    {MATRIX_ROLES.map(role => (
                      <th key={role} className="px-3 py-2 text-center border-l border-gray-100 min-w-[110px]">
                        <div className="space-y-1">
                        ...
                        return (
                          <td key={role} className="px-3 py-2 text-center border-l border-gray-100">
                            <input
                              ...
                              className="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer disabled:cursor-default disabled:opacity-60"
                              title={isAdmin ? 'Administrator always has full access' : `${checked ? 'Revoke' : 'Grant'} "${label}" for ${role}`}
                            />
                          </td>
                        );
                      })}
...
function UserPermissionsOverrides({ users, currentUser }: UserPermissionsOverridesProps) {
  ...
  const selectedOverride = selectedUserId ? userOverrides[selectedUserId] : null;
  ...
  const handleTogglePermission = async (permission: Permission) => {
    ...
    if (currentlyGranted) {
      ...
      updated.revokedPermissions = updated.revokedPermissions.filter(p => p !== permission);
  ...
  const handleApplyTemplate = async () => {
    if (!selectedTemplate || !selectedUserId || !selectedUser || !currentUser) return;

    await applyTemplateToUser(selectedTemplate, selectedUserId, selectedUser.name, {
      id: currentUser.id,
  ...
  const getPermissionStatus = (permission: Permission): 'granted' | 'revoked' | 'default' => {
    if (!selectedOverride) return 'default';
    if (selectedOverride.grantedPermissions.includes(permission)) return 'granted';
    if (selectedOverride.revokedPermissions.includes(permission)) return 'revoked';
    return 'default';
  };
  ...
  const handleGrantAll = async () => {
    ...
    toast.success(`Granted all ${ALL_PERMISSIONS.length} permissions to ${selectedUser.name}`);
  };
  ...
  const handleRevokeAll = async () => {
    ...
    toast.success(`Revoked all ${ALL_PERMISSIONS.length} permissions from ${selectedUser.name}`);
  };

  return (
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="text-sm font-medium text-blue-900 mb-2">💡 User-Level Permission Overrides</h3>
        <p className="text-sm text-blue-800">
          Grant or revoke specific permissions for individual users beyond their role permissions.
          Granted permissions are shown in <span className="text-green-700 font-semibold">green</span>,
          revoked permissions in <span className="text-red-700 font-semibold">red</span>, and
          default (from role) in <span className="text-gray-700 font-semibold">gray</span>.
        </p>
      </div>
        ...
        <div className="md:col-span-2">
          {selectedUser ? (
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="mb-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Base Role: {selectedUser.role}</p>
                  </div>
              ...
              <div className="max-h-[600px] overflow-y-auto space-y-2">
                {ALL_PERMISSIONS.map(permission => {
                  ...
                  const hasFromRole = hasPermission(selectedUser.role, permission);
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          ...
                          {status === 'default' && !hasFromRole && '○ Not Granted'}
                        </div>
                      </div>
                      <input
                        type="checkbox"
                        checked={status === 'granted' || (status === 'default' && hasFromRole)}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              ...
              <p className="text-sm text-gray-500">Select a user from the list to manage their permissions</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showTemplateModal && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50" onClick={() => { setShowTemplateModal(false); setSelectedTemplate(''); }}></div>
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-white dark:bg-gray-800 rounded-t-lg flex-shrink-0">
                <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Apply Permission Template</h3>
                <button
                  onClick={() => { setShowTemplateModal(false); setSelectedTemplate(''); }}
              ...
              <div className="p-6 flex-grow overflow-y-auto custom-scrollbar">
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Select a template to apply to {selectedUser?.name}. This will clear existing overrides and apply the template permissions.
                </p>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <h3 className="text-sm font-medium text-yellow-900 mb-2">🔍 Audit Trail</h3>
        <p className="text-sm text-yellow-800">
          Complete history of permission changes, role assignments, and security-related actions.
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {filteredLogs.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-700 mb-2">No Audit Logs</h3>
            <p className="text-sm text-gray-500">No permission changes have been logged yet</p>
          </div>
        ) : (
